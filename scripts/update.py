...
def uv_action(
    ...
    *,
    ...
    if verbose:
        ...
        if is_ci and action == "lock":
            print("\n" + "## UV updates" + "\n")
            _run_command(command, log_output=True)
        else:
            subprocess.check_call(command)
    else:
...
def run_pre_commit_autoupdate() -> None:
    results = _run_command("pre-commit autoupdate")
    messages = results.stdout.strip().split("\n")
    print("\n## Pre-commit updates\n")
    if has_update := [f"- {msg}" for msg in messages if "->" in msg]:
        print("\n".join(has_update))
    else:
        print("No pre-commit updates found.")
...
def main(
    ...
    run_uv_command(original_orders, verbose, ci)
    if not sort_deps:
